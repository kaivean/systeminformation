...
function calcDiskIO(rIO, wIO, rWaitTime, wWaitTime, tWaitTime) {
  ...
  if (_disk_io && _disk_io.ms) {
    ...
    result.rUtil = (result.rWaitTime - _disk_io.rWaitTime) * 100 / (result.ms);
    result.wUtil = (result.wWaitTime - _disk_io.wWaitTime) * 100 / (result.ms);
    result.tUtil = (result.tWaitTime - _disk_io.tWaitTime) * 100 / (result.ms);
    _disk_io.rIO = rIO;
    _disk_io.wIO = wIO;
    ...
    _disk_io.rUtil = result.rUtil;
    _disk_io.wUtil = result.wUtil;
    _disk_io.tUtil = result.tUtil;
  ...
  return new Promise((resolve) => {
    process.nextTick(() => {
      ...
      if ((_disk_io && !_disk_io.ms) || (_disk_io && _disk_io.ms && Date.now() - _disk_io.ms >= 500)) {
        ...
        result.rUtil = _disk_io.rUtil;
        result.wUtil = _disk_io.wUtil;
        result.tUtil = _disk_io.tUtil;
